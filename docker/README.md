...
Detailed instruction: see the `Makefile` in the root of [the repo](https://github.com/tabilabs/tabi-v2/blob/main/Makefile)
...
This will start a 4 node tabi chain cluster as well as having the oracle price feeder run for each node.
```sh
# If this is the first time or you want to rebuild the binary:
make docker-cluster-start

# If you have run docker-cluster-start and build/tabid exist,
...
tail -f build/generated/logs/tabid-0.log
```
...
```sh
# Clone your dependency repo and put them under the same path as tabi-chain
cd tabi-chain
cd ../
git clone https://github.com/tabilabs/tabi-tendermint.git
git clone https://github.com/tabilabs/tabi-cosmos.git

# Modify go.mod file to point to local repo, must use the exact same path as below:
go mod edit -replace github.com/cosmos/cosmos-sdk=../tabi-cosmos
go mod edit -replace github.com/tendermint/tendermint=../tabi-tendermint
