...
import (
	...
	"github.com/facjas/tabi-db/ss/types"
	"github.com/tabilabs/tabi-v2/tools/utils"
...
func (m *Migrator) Verify(version int64) error {
	...
	for _, module := range utils.Modules {
		...
		_, err = tree.Iterate(func(key []byte, value []byte) bool {
			...
			count++
			if count%1000000 == 0 {
				fmt.Printf("TabiDB Archive Migration: Verified %d keys in for module %s\n", count, module)
			}
			return false
		})
		if err != nil {
			fmt.Printf("TabiDB Archive Migration: Failed to iterate the tree %s: %s\n", module, err.Error())
			return err
		}
		fmt.Printf("TabiDB Archive Migration:: Finished verifying module %s, total scanned: %d keys\n", module, count)
	}
	return verifyErr
}
...
func ExportLeafNodesFromKey(db dbm.DB, ch chan<- types.RawSnapshotNode, startKey []byte, startModule string) error {
	count := 0
	leafNodeCount := 0
	fmt.Println("TabiDB Archive Migration: Scanning database and exporting leaf nodes...")
	...
	for _, module := range utils.Modules {
		...
		fmt.Printf("TabiDB Archive Migration: Iterating through %s module...\n", module)
		...
		if err != nil {
			fmt.Printf("TabiDB Archive Migration: Error creating iterator: %+v\n", err)
		...
		for ; itr.Valid(); itr.Next() {
			...
			if err != nil {
				fmt.Printf("TabiDB Archive Migration: Failed to make node: %+v\n", err)
			...
			if count%1000000 == 0 {
				...
				fmt.Printf("TabiDB Archive Migration: Last 1,000,000 iterations took: %v. Total scanned: %d, leaf nodes exported: %d\n", batchDuration, count, leafNodeCount)
				metrics.IncrCounterWithLabels([]string{"tabi", "migration", "leaf_nodes_exported"}, float32(batchLeafNodeCount), []metrics.Label{
		...
		fmt.Printf("TabiDB Archive Migration: Finished scanning module %s. Time taken: %v. Total scanned: %d, leaf nodes exported: %d\n", module, moduleDuration, count, leafNodeCount)
	}

	totalDuration := time.Since(startTimeTotal)
	fmt.Printf("TabiDB Archive Migration: DB scanning completed. Total time taken: %v. Total entries scanned: %d, leaf nodes exported: %d\n", totalDuration, count, leafNodeCount)
