...
import (
	...
	"github.com/facjas/tabi-db/config"
	sstypes "github.com/facjas/tabi-db/ss"
	"github.com/spf13/cobra"
	"github.com/tabilabs/tabi-v2/tools/migration/sc"
	"github.com/tabilabs/tabi-v2/tools/migration/ss"
...
func MigrateCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "migrate-iavl",
		Short: "A tool to migrate full IAVL data store to TabiDB. Use this tool to migrate IAVL to TabiDB SC and SS database.",
		Run:   execute,
	}
	cmd.PersistentFlags().String("home-dir", "/root/.tabi", "Tabi home directory")
	return cmd
}
...
func VerifyMigrationCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "A tool to verify migration of a IAVL data store to TabiDB at a particular height.",
		Run:   verify,
	}
...
func generateIavlStats(cmd *cobra.Command, _ []string) {
	...
	fmt.Println("TabiDB Archive Migration: Aggregation completed.")
}
