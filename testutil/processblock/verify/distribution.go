...
import (
	...
	"github.com/tabilabs/tabi-v2/testutil/processblock"
	"github.com/tabilabs/tabi-v2/utils"
)

// assuming only `atabi` will get distributed
func Allocation(t *testing.T, app *processblock.App, f BlockRunnable, _ []signing.Tx) BlockRunnable {
	return func() []uint32 {
		...
		feesCollectedInt := app.BankKeeper.GetBalance(app.Ctx(), feeCollector.GetAddress(), "atabi")
...
func getOutstandingRewards(app *processblock.App) map[string]sdk.DecCoin {
	...
	for _, val := range app.GetAllValidators() {
		outstandingRewards[val.GetOperator().String()] = sdk.NewDecCoin("atabi", sdk.NewInt(0))
	}
	app.DistrKeeper.IterateValidatorOutstandingRewards(
		...
		func(val sdk.ValAddress, rewards types.ValidatorOutstandingRewards) (stop bool) {
			...
			if len(rewards.Rewards) > 1 {
				panic("expecting only atabias validator reward denom but found multiple")
			}
			if rewards.Rewards[0].Denom != "atabi" {
				panic(fmt.Sprintf("expecting only atabias validator reward denom but found %s", rewards.Rewards[0].Denom))
			}
