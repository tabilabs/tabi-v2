...
func MockEVMKeeperWithPrecompiles() (*evmkeeper.Keeper, sdk.Context) {
	...
	// mint some coins to a tabi address
	tabiAddr, err := sdk.AccAddressFromHex(common.Bytes2Hex([]byte("tabiAddr")))
	if err != nil {
		panic(err)
	}
	err = EVMTestApp.BankKeeper.MintCoins(ctx, "evm", sdk.NewCoins(sdk.NewCoin("atabi", sdk.NewInt(10))))
	if err != nil {
		panic(err)
	}
	err = EVMTestApp.BankKeeper.SendCoinsFromModuleToAccount(ctx, "evm", tabiAddr, sdk.NewCoins(sdk.NewCoin("atabi", sdk.NewInt(10))))
	if err != nil {
		panic(err)
	}
	mockKeeper = &k
...
func MockEVMKeeper() (*evmkeeper.Keeper, sdk.Context) {
	...
	err = testApp.BankKeeper.MintCoins(ctx, "evm", sdk.NewCoins(sdk.NewCoin("atabi", sdk.NewInt(10))))
	if err != nil {
		panic(err)
	}
	err = testApp.BankKeeper.SendCoinsFromModuleToAccount(ctx, "evm", tabiAddr, sdk.NewCoins(sdk.NewCoin("atabi", sdk.NewInt(10))))
	if err != nil {
		panic(err)
	}
	return &k, ctx
...
func MockPrivateKey() cryptotypes.PrivKey {
	// Generate a new Tabi private key
...
func AtabiCoins(amount int64) sdk.Coins {
