...
func TransactionDependencyGenerator(_ aclkeeper.Keeper, evmKeeper evmkeeper.Keeper, ctx sdk.Context, msg sdk.Msg) ([]sdkacltypes.AccessOperation, error) {
	...
	sender := evmMsg.Derived.SenderTabiAddr
	...
	toAddress := tx.To()
	if toAddress != nil {
		tabiAddress := evmKeeper.GetTabiAddressOrDefault(ctx, *toAddress)
		ops = appendRWBalanceOps(ops, tabiAddress)
		ops = append(ops, sdkacltypes.AccessOperation{
			AccessType:         sdkacltypes.AccessType_READ,
			ResourceType:       sdkacltypes.ResourceType_KV_EVM_E2S,
			IdentifierTemplate: hex.EncodeToString(evmtypes.EVMAddressToTabiAddressKey(*toAddress)),
		}, sdkacltypes.AccessOperation{
			AccessType:         sdkacltypes.AccessType_READ,
	...
	return append(ops, []sdkacltypes.AccessOperation{
		...
		},
		{
			...
			IdentifierTemplate: hex.EncodeToString(evmtypes.TabiAddressToEVMAddressKey(evmKeeper.AccountKeeper().GetModuleAddress(authtypes.FeeCollectorName))),
		},
		{
			AccessType:         sdkacltypes.AccessType_READ,
			ResourceType:       sdkacltypes.ResourceType_KV_EVM_E2S,
			IdentifierTemplate: hex.EncodeToString(evmtypes.EVMAddressToTabiAddressKey(evmAddr)),
		},
		{
			AccessType:         sdkacltypes.AccessType_READ,
			ResourceType:       sdkacltypes.ResourceType_KV_EVM_S2E,
			IdentifierTemplate: hex.EncodeToString(evmtypes.TabiAddressToEVMAddressKey(evmMsg.Derived.SenderTabiAddr)),
		},
		{
			AccessType:         sdkacltypes.AccessType_READ,
			ResourceType:       sdkacltypes.ResourceType_KV_EVM_CODE_HASH,
