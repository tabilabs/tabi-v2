...
The association between the tabi address and the eth address will also be created here if using keyring-backend test.
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if keyringBackend == keyring.BackendTest {
				// associate the eth address with the tabi address through the genesis file
				evmGenState := evm.GetGenesisStateFromAppState(depCdc, appState)
				tabiEthAddrAssociation := evmtypes.AddressAssociation{
					TabiAddress: addr.String(),
					EthAddress:  ethAddr.Hex(),
				}
				evmGenState.AddressAssociations = append(evmGenState.AddressAssociations, &tabiEthAddrAssociation)
...
func getPrivateKeyOfAddr(kb keyring.Keyring, addr sdk.Address) (*ecdsa.PrivateKey, error) {
	...
	for _, key := range keys {
		...
		if localInfo.GetAddress().Equals(addr) {
			...
			// Need to use private key to convert to tabi address here
