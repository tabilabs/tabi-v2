		...
		case "increase_allowance", "decrease_allowance":
			...
			ret, err := app.WasmKeeper.QuerySmart(
				...
				[]byte(fmt.Sprintf(
					...
					app.EvmKeeper.GetTabiAddressOrDefault(ctx, common.BytesToAddress(action.Owner[:])).String(),
					app.EvmKeeper.GetTabiAddressOrDefault(ctx, common.BytesToAddress(action.Spender[:])).String())),
			)
			if err != nil {
				continue
			}
		...
		case "transfer_nft", "send_nft", "burn":
			...
			if ownerEvents, ok := ownerEventsMap[ownerEventKey]; ok {
				if len(ownerEvents) > currentCounter {
					ownerTabiAddrStr := string(ownerEvents[currentCounter].Attributes[2].Value)
					if ownerTabiAddr, err := sdk.AccAddressFromBech32(ownerTabiAddrStr); err == nil {
						ownerEvmAddr := app.EvmKeeper.GetEVMAddressOrDefault(ctx, ownerTabiAddr)
						sender = common.BytesToHash(ownerEvmAddr[:])
					} else {
						ctx.Logger().Error("Translate CW721 error: invalid bech32 owner", "error", err, "address", ownerTabiAddrStr)
					}
				} else {
...
func (app *App) GetEvmAddressHash(ctx sdk.Context, addrStr string) common.Hash {
	tabiAddr, err := sdk.AccAddressFromBech32(addrStr)
	if err == nil {
		evmAddr := app.EvmKeeper.GetEVMAddressOrDefault(ctx, tabiAddr)
