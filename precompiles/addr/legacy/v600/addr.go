...
import (
	...
	helpers "github.com/tabilabs/tabi-v2/utils/helpers/legacy/v600"
...
func (p PrecompileExecutor) associateAddresses(ctx sdk.Context, method *abi.Method, evmAddr common.Address, tabiAddr sdk.AccAddress, pubkey cryptotypes.PubKey) ([]byte, error) {
	// Check that address is not already associated
	_, found := p.evmKeeper.GetEVMAddress(ctx, tabiAddr)
	if found {
		return nil, fmt.Errorf("address %s is already associated with evm address %s", tabiAddr, evmAddr)
	}
	...
	return method.Outputs.Pack(tabiAddr.String(), evmAddr)
}
