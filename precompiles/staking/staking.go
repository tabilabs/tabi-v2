...
func (p PrecompileExecutor) delegate(ctx sdk.Context, method *abi.Method, caller common.Address, args []interface{}, value *big.Int, hooks *tracing.Hooks, evm *vm.EVM) ([]byte, error) {
	...
	delegator, associated := p.evmKeeper.GetTabiAddress(ctx, caller)
	if !associated {
	...
	coin, err := pcommon.HandlePaymentUtabi(ctx, p.evmKeeper.GetTabiAddressOrDefault(ctx, p.address), delegator, value, p.bankKeeper, p.evmKeeper, hooks, evm.GetDepth())
	if err != nil {
		return nil, err
	}
...
func (p PrecompileExecutor) delegation(ctx sdk.Context, method *abi.Method, args []interface{}, value *big.Int) ([]byte, error) {
	...
	tabiDelegatorAddress, err := pcommon.GetTabiAddressFromArg(ctx, args[0], p.evmKeeper)
	if err != nil {
		return nil, err
	}
	...
	delegationRequest := &stakingtypes.QueryDelegationRequest{
		DelegatorAddr: tabiDelegatorAddress.String(),
...
func (p PrecompileExecutor) createValidator(ctx sdk.Context, method *abi.Method, caller common.Address, args []interface{}, value *big.Int, hooks *tracing.Hooks, evm *vm.EVM) ([]byte, error) {
	...
	// Get validator address (caller's associated Tabi address)
	valAddress, associated := p.evmKeeper.GetTabiAddress(ctx, caller)
	if !associated {
		return nil, types.NewAssociationMissingErr(caller.Hex())
	}
	...
	coin, err := pcommon.HandlePaymentUtabi(
		ctx,
		p.evmKeeper.GetTabiAddressOrDefault(ctx, p.address),
		valAddress,
		value,
		p.bankKeeper,
		p.evmKeeper,
