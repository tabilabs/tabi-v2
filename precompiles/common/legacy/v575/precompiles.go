...
func HandlePaymentUtabi(ctx sdk.Context, precompileAddr sdk.AccAddress, payer sdk.AccAddress, value *big.Int, bankKeeper BankKeeper, evmKeeper EVMKeeper, hooks *tracing.Hooks, depth int) (sdk.Coin, error) {
	atabi, wei := state.SplitAtabiWeiAmount(value)
	if !wei.IsZero() {
	...
	coin := sdk.NewCoin(sdk.MustGetBaseDenom(), atabi)
...
func HandlePaymentUtabiWei(ctx sdk.Context, precompileAddr sdk.AccAddress, payer sdk.AccAddress, value *big.Int, bankKeeper BankKeeper, evmKeeper EVMKeeper, hooks *tracing.Hooks, depth int) (sdk.Int, sdk.Int, error) {
	atabi, wei := state.SplitAtabiWeiAmount(value)
	// refund payer because the following precompile logic will debit the payments from payer's account
	// this creates a new event manager to avoid surfacing these as cosmos events
	if err := bankKeeper.SendCoinsAndWei(ctx.WithEventManager(sdk.NewEventManager()), precompileAddr, payer, atabi, wei); err != nil {
	...
	return atabi, wei, nil
}

/*
*
tabi gas = evm gas * multiplier
tabi gas price = fee / tabi gas = fee / (evm gas * multiplier) = evm gas / multiplier
*/
func GetRemainingGas(ctx sdk.Context, evmKeeper EVMKeeper) uint64 {
	gasMultipler := evmKeeper.GetPriorityNormalizerPre580(ctx)
	tabiGasRemaining := ctx.GasMeter().Limit() - ctx.GasMeter().GasConsumedToLimit()
	return sdk.NewDecFromInt(sdk.NewIntFromUint64(tabiGasRemaining)).Quo(gasMultipler).TruncateInt().Uint64()
}
...
func GetTabiAddressByEvmAddress(ctx sdk.Context, evmAddress common.Address, evmKeeper EVMKeeper) (sdk.AccAddress, error) {
	tabiAddr, associated := evmKeeper.GetTabiAddress(ctx, evmAddress)
	if !associated {
...
func GetTabiAddressFromArg(ctx sdk.Context, arg interface{}, evmKeeper EVMKeeper) (sdk.AccAddress, error) {
	addr := arg.(common.Address)
	if addr == (common.Address{}) {
		return nil, errors.New("invalid addr")
	}
	return GetTabiAddressByEvmAddress(ctx, addr, evmKeeper)
}
