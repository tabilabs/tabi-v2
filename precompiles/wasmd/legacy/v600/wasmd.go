...
import (
	...
	pcommon "github.com/tabilabs/tabi-v2/precompiles/common/legacy/v600"
...
func (p PrecompileExecutor) Execute(ctx sdk.Context, method *abi.Method, caller common.Address, callingContract common.Address, args []interface{}, value *big.Int, readOnly bool, evm *vm.EVM, suppliedGas uint64, hooks *tracing.Hooks) (ret []byte, remainingGas uint64, err error) {
	if method.Name != QueryMethod && !ctx.IsEVM() {
		return nil, 0, errors.New("tabi does not support CW->EVM->CW call pattern")
	}
	switch method.Name {
	case InstantiateMethod:
...
func (p PrecompileExecutor) instantiate(ctx sdk.Context, method *abi.Method, caller common.Address, _ common.Address, args []interface{}, value *big.Int, readOnly bool, hooks *tracing.Hooks, evm *vm.EVM) (ret []byte, remainingGas uint64, rerr error) {
	...
	creatorAddr, found := p.evmKeeper.GetTabiAddress(ctx, caller)
	if !found {
	...
	coinsValue := coins.AmountOf(sdk.MustGetBaseDenom()).Mul(state.SdkAtabiToWeiMultiplier).BigInt()
	...
	atabiAmt := coins.AmountOf(sdk.MustGetBaseDenom())
	if value != nil && !atabiAmt.IsZero() {
		atabiAmtAsWei := atabiAmt.Mul(state.SdkAtabiToWeiMultiplier).BigInt()
		coin, err := pcommon.HandlePaymentUtabi(ctx, p.evmKeeper.GetTabiAddressOrDefault(ctx, p.address), creatorAddr, atabiAmtAsWei, p.bankKeeper, p.evmKeeper, hooks, evm.GetDepth())
		if err != nil {
			rerr = err
			return
		}
		// sanity check coin amounts match
		if !coin.Amount.Equal(atabiAmt) {
...
func (p PrecompileExecutor) executeBatch(ctx sdk.Context, method *abi.Method, caller common.Address, callingContract common.Address, args []interface{}, value *big.Int, readOnly bool, hooks *tracing.Hooks, evm *vm.EVM) (ret []byte, remainingGas uint64, rerr error) {
	...
	for i := 0; i < len(executeMsgs); i++ {
		...
		messageAmount := coins.AmountOf(sdk.MustGetBaseDenom()).Mul(state.SdkAtabiToWeiMultiplier).BigInt()
	...
	for i := 0; i < len(executeMsgs); i++ {
		...
		senderAddr, senderAssociated := p.evmKeeper.GetTabiAddress(ctx, caller)
		if !senderAssociated {
			rerr = types.NewAssociationMissingErr(caller.Hex())
			return
		}
		...
		atabiAmt := coins.AmountOf(sdk.MustGetBaseDenom())
		if valueCopy != nil && !atabiAmt.IsZero() {
			// process coin amount from the value provided
			atabiAmtAsWei := atabiAmt.Mul(state.SdkAtabiToWeiMultiplier).BigInt()
			coin, err := pcommon.HandlePaymentUtabi(ctx, p.evmKeeper.GetTabiAddressOrDefault(ctx, p.address), senderAddr, atabiAmtAsWei, p.bankKeeper, p.evmKeeper, hooks, evm.GetDepth())
			if err != nil {
				rerr = err
				return
			}
			valueCopy.Sub(valueCopy, atabiAmtAsWei)
			if valueCopy.Sign() == -1 {
...
func (p PrecompileExecutor) execute(ctx sdk.Context, method *abi.Method, caller common.Address, callingContract common.Address, args []interface{}, value *big.Int, readOnly bool, hooks *tracing.Hooks, evm *vm.EVM) (ret []byte, remainingGas uint64, rerr error) {
	...
	// addresses will be sent in Tabi format
	contractAddr, err := sdk.AccAddressFromBech32(contractAddrStr)
	if err != nil {
		rerr = err
		return
	}
	senderAddr, found := p.evmKeeper.GetTabiAddress(ctx, caller)
	if !found {
		rerr = types.NewAssociationMissingErr(caller.Hex())
		return
	}
