...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=tabi \
			-X github.com/cosmos/cosmos-sdk/version.ServerName=tabid \
...
install: go.sum
		go install $(BUILD_FLAGS) ./cmd/tabid

install-with-race-detector: go.sum
		go install -race $(BUILD_FLAGS) ./cmd/tabid
...
build:
	go build $(BUILD_FLAGS) -o ./build/tabid ./cmd/tabid
...
# Build docker image
build-docker-node:
	@cd docker && docker build --tag tabi-v2/localnode localnode --platform linux/x86_64
.PHONY: build-docker-node

build-rpc-node:
	@cd docker && docker build --tag tabi-v2/rpcnode rpcnode --platform linux/x86_64
.PHONY: build-rpc-node

# Run a single node docker container
run-local-node: kill-tabi-node build-docker-node
	@rm -rf $(PROJECT_HOME)/build/generated
	docker run --rm \
	--name tabi-node \
	--network host \
	--user="$(shell id -u):$(shell id -g)" \
	-v $(PROJECT_HOME):/tabilabs/tabi-v2:Z \
...
run-rpc-node: build-rpc-node
	...
	--user="$(shell id -u):$(shell id -g)" \
	-v $(PROJECT_HOME):/tabilabs/tabi-v2:Z \
	-v $(PROJECT_HOME)/../tabi-tendermint:/tabilabs/tabi-tendermint:Z \
    -v $(PROJECT_HOME)/../tabi-cosmos:/tabilabs/tabi-v2:Z \
    -v $(PROJECT_HOME)/../tabi-db:/tabilabs/tabi-db:Z \
	-v $(GO_PKG_PATH)/mod:/root/go/pkg/mod:Z \
	-v $(shell go env GOCACHE):/root/.cache/go-build:Z \
...
run-rpc-node-skipbuild: build-rpc-node
	...
	-v $(PROJECT_HOME):/tabilabs/tabi-v2:Z \
	-v $(PROJECT_HOME)/../tabi-tendermint:/tabilabs/tabi-tendermint:Z \
    -v $(PROJECT_HOME)/../tabi-cosmos:/tabilabs/tabi-cosmos:Z \
    -v $(PROJECT_HOME)/../tabi-db:/tabilabs/tabi-db:Z \
	-v $(GO_PKG_PATH)/mod:/root/go/pkg/mod:Z \
	-v $(shell go env GOCACHE):/root/.cache/go-build:Z \
	-p 26668-26670:26656-26658 \
...
kill-tabi-node:
	docker ps --filter name=tabi-node --filter status=running -aq | xargs docker kill 2> /dev/null || true

kill-rpc-node:
	docker ps --filter name=tabi-rpc-node --filter status=running -aq | xargs docker kill 2> /dev/null || true
...
# Use this to skip the tabid build process
