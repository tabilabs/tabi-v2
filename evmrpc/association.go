...
func (t *AssociationAPI) Associate(ctx context.Context, req *AssociateRequest) (returnErr error) {
	startTime := time.Now()
	defer recordMetrics("tabi_associate", t.connectionType, startTime, returnErr == nil)
...
func (t *AssociationAPI) GetTabiAddress(_ context.Context, ethAddress common.Address) (result string, returnErr error) {
	startTime := time.Now()
	defer recordMetrics("tabi_getTabiAddress", t.connectionType, startTime, returnErr == nil)
	tabiAddress, found := t.keeper.GetTabiAddress(t.ctxProvider(LatestCtxHeight), ethAddress)
	if !found {
		return "", fmt.Errorf("failed to find Tabi address for %s", ethAddress.Hex())
	}

	return tabiAddress.String(), nil
}

func (t *AssociationAPI) GetEVMAddress(_ context.Context, tabiAddress string) (result string, returnErr error) {
	startTime := time.Now()
	defer recordMetrics("tabi_getEVMAddress", t.connectionType, startTime, returnErr == nil)
	tabiAddr, err := sdk.AccAddressFromBech32(tabiAddress)
	if err != nil {
		return "", err
	}
	ethAddress, found := t.keeper.GetEVMAddress(t.ctxProvider(LatestCtxHeight), tabiAddr)
	if !found {
		return "", fmt.Errorf("failed to find EVM address for %s", tabiAddress)
	}
...
func (t *AssociationAPI) GetCosmosTx(ctx context.Context, ethHash common.Hash) (result string, returnErr error) {
	startTime := time.Now()
	defer recordMetrics("tabi_getCosmosTx", t.connectionType, startTime, returnErr == nil)
...
func (t *AssociationAPI) GetEvmTx(ctx context.Context, cosmosHash string) (result string, returnErr error) {
	startTime := time.Now()
	defer recordMetrics("tabi_getEvmTx", t.connectionType, startTime, returnErr == nil)
