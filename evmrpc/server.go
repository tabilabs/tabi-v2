...
import (
	...
	evmCfg "github.com/tabilabs/tabi-v2/x/evm/config"
...
func NewEVMHTTPServer(
	...
	tabiTxAPI := NewTabiTransactionAPI(tmClient, k, ctxProvider, txConfigProvider, homeDir, ConnectionTypeHTTP, isPanicOrSyntheticTxFunc)
	tabiDebugAPI := NewTabiDebugAPI(tmClient, k, ctxProvider, txConfigProvider, simulateConfig, app, antehandler, ConnectionTypeHTTP, config)
	...
	apis := []rpc.API{
		...
		{
			Namespace: "tabi",
			Service:   NewTabiBlockAPI(tmClient, k, ctxProvider, txConfigProvider, ConnectionTypeHTTP, isPanicOrSyntheticTxFunc),
		},
		{
			Namespace: "tabi2",
			Service:   NewTabi2BlockAPI(tmClient, k, ctxProvider, txConfigProvider, ConnectionTypeHTTP, isPanicOrSyntheticTxFunc),
		},
		{
			Namespace: "eth",
			Service:   txAPI,
		...
		{
			Namespace: "tabi",
			Service:   NewFilterAPI(tmClient, k, ctxProvider, txConfigProvider, &FilterConfig{timeout: config.FilterTimeout, maxLog: config.MaxLogNoBlock, maxBlock: config.MaxBlocksForLog}, ConnectionTypeHTTP, "tabi"),
		},
		{
			Namespace: "tabi",
