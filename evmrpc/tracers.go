...
func NewTabiDebugAPI(
	...
	embeddedDebugAPI := &DebugAPI{
		...
		maxBlockLookback:   debugCfg.MaxTraceLookbackBlocks,
		traceTimeout:       debugCfg.TraceTimeout,
		// isPanicCache: nil, // Explicitly nil as per original structure for TabiDebugAPI's embedded DebugAPI
	}
...
func (api *TabiDebugAPI) TraceBlockByNumberExcludeTraceFail(ctx context.Context, number rpc.BlockNumber, config *tracers.TraceConfig) (result interface{}, returnErr error) {
	...
	startTime := time.Now()
	defer recordMetrics("tabi_traceBlockByNumberExcludeTraceFail", api.connectionType, startTime, returnErr == nil)
...
func (api *TabiDebugAPI) TraceBlockByHashExcludeTraceFail(ctx context.Context, hash common.Hash, config *tracers.TraceConfig) (result interface{}, returnErr error) {
	release := api.acquireTraceSemaphore() // Use the embedded DebugAPI's semaphore
	defer release()
	...
	startTime := time.Now()
	defer recordMetrics("tabi_traceBlockByHashExcludeTraceFail", api.connectionType, startTime, returnErr == nil)
	// Accessing tracersAPI from the embedded DebugAPI
