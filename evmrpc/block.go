...
func NewTabi2BlockAPI(
	...
	blockAPI := NewTabiBlockAPI(tmClient, k, ctxProvider, txConfigProvider, connectionType, isPanicTx)
	blockAPI.namespace = "tabi2"
	blockAPI.includeBankTransfers = true
	return blockAPI
}

func (a *TabiBlockAPI) GetBlockByNumberExcludeTraceFail(ctx context.Context, number rpc.BlockNumber, fullTx bool) (result map[string]interface{}, returnErr error) {
	// exclude synthetic txs
	return a.getBlockByNumber(ctx, number, fullTx, false, a.isPanicTx)
}

func (a *TabiBlockAPI) GetBlockByHashExcludeTraceFail(ctx context.Context, blockHash common.Hash, fullTx bool) (result map[string]interface{}, returnErr error) {
	// exclude synthetic txs
...
func (a *BlockAPI) GetBlockByHash(ctx context.Context, blockHash common.Hash, fullTx bool) (result map[string]interface{}, returnErr error) {
	// used for both: eth_ and tabi_ namespaces
...
func (a *BlockAPI) GetBlockByNumber(ctx context.Context, number rpc.BlockNumber, fullTx bool) (result map[string]interface{}, returnErr error) {
	...
	if number == 0 {
		...
		return map[string]interface{}{
			...
			"nonce":            ethtypes.BlockNonce{},   // inapplicable to Tabi
			"mixHash":          common.Hash{},           // inapplicable to Tabi
			"sha3Uncles":       ethtypes.EmptyUncleHash, // inapplicable to Tabi
			"logsBloom":        ethtypes.Bloom{},
			"stateRoot":        common.Hash{},
			"miner":            common.Address{},
			"difficulty":       (*hexutil.Big)(big.NewInt(0)), // inapplicable to Tabi
			...
			"uncles":           []common.Hash{}, // inapplicable to Tabi
...
func EncodeTmBlock(
		...
		for _, msg := range decoded.GetMsgs() {
			...
			case *banktypes.MsgSend:
				...
				if !fullTx {
					...
					senderTabiAddr, err := sdk.AccAddressFromBech32(m.FromAddress)
					if err != nil {
						continue
					}
					rpcTx.From = k.GetEVMAddressOrDefault(ctx, senderTabiAddr)
					recipientTabiAddr, err := sdk.AccAddressFromBech32(m.ToAddress)
					if err != nil {
						continue
					}
					recipientEvmAddr := k.GetEVMAddressOrDefault(ctx, recipientTabiAddr)
					rpcTx.To = &recipientEvmAddr
					amt := m.Amount.AmountOf("atabi").Mul(state.SdkAtabiToWeiMultiplier)
	...
	if fullTx {
		result["totalDifficulty"] = (*hexutil.Big)(big.NewInt(0)) // inapplicable to Tabi
	}
	return result, nil
}
