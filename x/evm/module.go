...
import (
	...
	"github.com/tabilabs/tabi-v2/x/evm/client/cli"
...
func (am AppModule) EndBlock(ctx sdk.Context, req abci.RequestEndBlock) []abci.ValidatorUpdate {
	...
	if am.keeper.EthBlockTestConfig.Enabled {
		...
		coinbase = am.keeper.GetTabiAddressOrDefault(ctx, block.Header_.Coinbase)
	} else if am.keeper.EthReplayConfig.Enabled {
		coinbase = am.keeper.GetTabiAddressOrDefault(ctx, am.keeper.ReplayBlock.Header_.Coinbase)
	...
	for _, deferredInfo := range evmTxDeferredInfoList {
		...
		atabiBalance := am.keeper.BankKeeper().GetBalance(ctx, coinbaseAddress, denom).Amount
		lockedAtabiBalance := am.keeper.BankKeeper().LockedCoins(ctx, coinbaseAddress).AmountOf(denom)
		balance := atabiBalance.Sub(lockedAtabiBalance)
		...
		if !balance.IsZero() || !weiBalance.IsZero() {
			if err := am.keeper.BankKeeper().SendCoinsAndWei(ctx, coinbaseAddress, coinbase, balance, weiBalance); err != nil {
				ctx.Logger().Error(fmt.Sprintf("failed to send atabisurplus from %s to coinbase account due to %s", coinbaseAddress.String(), err))
			}
		}
		surplus = surplus.Add(deferredInfo.Surplus)
	}
	if surplus.IsPositive() {
		surplusAtabi, surplusWei := state.SplitAtabiWeiAmount(surplus.BigInt())
		if surplusAtabi.GT(sdk.ZeroInt()) {
			if err := am.keeper.BankKeeper().AddCoins(ctx, am.keeper.AccountKeeper().GetModuleAddress(types.ModuleName), sdk.NewCoins(sdk.NewCoin(am.keeper.GetBaseDenom(ctx), surplusAtabi)), true); err != nil {
				ctx.Logger().Error("failed to send atabisurplus of %s to EVM module account", surplusAtabi)
			}
		}
