...
import (
	...
	"github.com/tabilabs/tabi-v2/x/evm/artifacts/wtabi"
...
func CmdAssociateAddress() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "associate EVM and Tabi address for the sender",
		Long:  "",
		...
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			...
			body := fmt.Sprintf("{\"jsonrpc\": \"2.0\",\"method\": \"tabi_associate\",\"params\":[%s],\"id\":\"associate_addr\"}", string(bz))
...
func CmdSend() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "send atabito EVM address",
		Long:  "",
...
func CmdDeployWSEI() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "deploy-wtabi --from=<sender> --gas-fee-cap=<cap> --gas-limt=<limit> --evm-rpc=<url>",
		Short: "Deploy ERC20 contract for a native Tabi token",
		Long:  "",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			contractData := wtabi.GetBin()
