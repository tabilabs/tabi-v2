...
import (
	...
	"github.com/tabilabs/tabi-v2/example/contracts/simplestorage"
	testkeeper "github.com/tabilabs/tabi-v2/testutil/keeper"
	"github.com/tabilabs/tabi-v2/x/evm/ante"
	"github.com/tabilabs/tabi-v2/x/evm/client/wasm"
	"github.com/tabilabs/tabi-v2/x/evm/client/wasm/bindings"
...
func TestHandleERC721RoyaltyInfo(t *testing.T) {
	...
	require.Nil(t, err)
	require.NotEmpty(t, res2)
	match, _ := regexp.MatchString(`{"receiver":"tabi\w{39}","royalty_amount":"5"}`, string(res2))
...
func TestGetAddress(t *testing.T) {
	...
	k.SetAddressMapping(ctx, tabiAddr1, evmAddr1)
	...
	res, err := h.HandleGetEvmAddress(ctx, tabiAddr1.String())
	require.Nil(t, err)
	...
	getEvmAddrResp = &bindings.GetEvmAddressResponse{}
	res, err = h.HandleGetEvmAddress(ctx, tabiAddr2.String())
	require.Nil(t, err)
	require.Nil(t, json.Unmarshal(res, getEvmAddrResp))
	require.True(t, getEvmAddrResp.Associated)
	getTabiAddrResp := &bindings.GetTabiAddressResponse{}
	res, err = h.HandleGetTabiAddress(ctx, evmAddr1.Hex())
	require.Nil(t, err)
	require.Nil(t, json.Unmarshal(res, getTabiAddrResp))
	require.True(t, getTabiAddrResp.Associated)
	require.Equal(t, tabiAddr1.String(), getTabiAddrResp.TabiAddress)
	getTabiAddrResp = &bindings.GetTabiAddressResponse{}
	res, err = h.HandleGetTabiAddress(ctx, evmAddr2.Hex())
	require.Nil(t, err)
	require.Nil(t, json.Unmarshal(res, getTabiAddrResp))
	require.True(t, getTabiAddrResp.Associated)
}
