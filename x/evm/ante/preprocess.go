...
import (
	...
	"github.com/tabilabs/tabi-v2/utils/helpers"
	...
	"github.com/tabilabs/tabi-v2/x/evm/derived"
...
func (p *EVMPreprocessDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (sdk.Context, error) {
	...
	derived := msg.Derived
	tabiAddr := derived.SenderTabiAddr
	...
	ctx.EventManager().EmitEvent(sdk.NewEvent(evmtypes.EventTypeSigner,
		...
		sdk.NewAttribute(evmtypes.AttributeKeyTabiAddress, tabiAddr.String())))
	...
	_, isAssociated := p.evmKeeper.GetEVMAddress(ctx, tabiAddr)
...
func (p *EVMPreprocessDecorator) IsAccountBalancePositive(ctx sdk.Context, tabiAddr sdk.AccAddress, evmAddr common.Address) bool {
	baseDenom := p.evmKeeper.GetBaseDenom(ctx)
	if amt := p.evmKeeper.BankKeeper().GetBalance(ctx, tabiAddr, baseDenom).Amount; amt.IsPositive() {
		return true
	}
	if amt := p.evmKeeper.BankKeeper().GetBalance(ctx, sdk.AccAddress(evmAddr[:]), baseDenom).Amount; amt.IsPositive() {
		return true
	}
	if amt := p.evmKeeper.BankKeeper().GetWeiBalance(ctx, tabiAddr); amt.IsPositive() {
		return true
	}
...
func Preprocess(ctx sdk.Context, msgEVMTransaction *evmtypes.MsgEVMTransaction) error {
	...
	if atx, ok := txData.(*ethtx.AssociateTx); ok {
		...
		evmAddr, tabiAddr, pubkey, err := helpers.GetAddresses(V, R, S, customMessageHash)
		if err != nil {
			return err
		}
		msgEVMTransaction.Derived = &derived.Derived{
			SenderEVMAddr:  evmAddr,
			SenderTabiAddr: tabiAddr,
	...
	evmAddr, tabiAddr, tabiPubkey, err := helpers.GetAddresses(V, R, S, txHash)
	if err != nil {
		return err
	}
	msgEVMTransaction.Derived = &derived.Derived{
		SenderEVMAddr:  evmAddr,
		SenderTabiAddr: tabiAddr,
		PubKey:         &secp256k1.PubKey{Key: tabiPubkey.Bytes()},
		Version:        version,
		IsAssociate:    false,
	}
	return nil
...
func (p *EVMPreprocessDecorator) AnteDeps(txDeps []sdkacltypes.AccessOperation, tx sdk.Tx, txIndex int, next sdk.AnteDepGenerator) (newTxDeps []sdkacltypes.AccessOperation, err error) {
	...
	return next(append(txDeps, sdkacltypes.AccessOperation{
		...
		IdentifierTemplate: hex.EncodeToString(evmtypes.TabiAddressToEVMAddressKey(msg.Derived.SenderTabiAddr)),
		...
		IdentifierTemplate: hex.EncodeToString(evmtypes.EVMAddressToTabiAddressKey(msg.Derived.SenderEVMAddr)),
	}, sdkacltypes.AccessOperation{
		AccessType:         sdkacltypes.AccessType_READ,
		ResourceType:       sdkacltypes.ResourceType_KV_BANK_BALANCES,
		IdentifierTemplate: hex.EncodeToString(banktypes.CreateAccountBalancesPrefix(msg.Derived.SenderTabiAddr)),
	}, sdkacltypes.AccessOperation{
		AccessType:         sdkacltypes.AccessType_WRITE,
		ResourceType:       sdkacltypes.ResourceType_KV_BANK_BALANCES,
		IdentifierTemplate: hex.EncodeToString(banktypes.CreateAccountBalancesPrefix(msg.Derived.SenderTabiAddr)),
		...
		IdentifierTemplate: hex.EncodeToString(authtypes.AddressStoreKey(msg.Derived.SenderTabiAddr)),
	}, sdkacltypes.AccessOperation{
		AccessType:         sdkacltypes.AccessType_WRITE,
		ResourceType:       sdkacltypes.ResourceType_KV_AUTH_ADDRESS_STORE,
		IdentifierTemplate: hex.EncodeToString(authtypes.AddressStoreKey(msg.Derived.SenderTabiAddr)),
...
func (p *EVMAddressDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (sdk.Context, error) {
	...
	for _, signer := range signers {
		if evmAddr, associated := p.evmKeeper.GetEVMAddress(ctx, signer); associated {
			ctx.EventManager().EmitEvent(sdk.NewEvent(evmtypes.EventTypeSigner,
				sdk.NewAttribute(evmtypes.AttributeKeyEvmAddress, evmAddr.Hex()),
				sdk.NewAttribute(evmtypes.AttributeKeyTabiAddress, signer.String())))
			continue
		}
