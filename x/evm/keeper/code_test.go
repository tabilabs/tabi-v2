...
func TestCode(t *testing.T) {
	...
	k.BankKeeper().MintCoins(ctx, "evm", sdk.NewCoins(sdk.NewCoin("atabi", sdk.OneInt())))
	k.BankKeeper().SendCoinsFromModuleToAccount(ctx, "evm", sdk.AccAddress(addr[:]), sdk.NewCoins(sdk.NewCoin("atabi", sdk.OneInt())))
	...
	require.Equal(t, sdk.AccAddress(addr[:]), k.AccountKeeper().GetAccount(ctx, k.GetTabiAddressOrDefault(ctx, addr)).GetAddress())
}
