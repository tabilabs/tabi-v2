...
import (
	...
	tabidbtypes "github.com/facjas/tabi-db/ss/types"
...
type Keeper struct {
	...
	receiptStore tabidbtypes.StateStore
...
func NewKeeper(
	storeKey sdk.StoreKey, transientStoreKey sdk.StoreKey, paramstore paramtypes.Subspace, receiptStateStore tabidbtypes.StateStore,
...
func (k *Keeper) GetHashFn(ctx sdk.Context) vm.GetHashFunc {
	return func(height uint64) common.Hash {
		if height > math.MaxInt64 {
			ctx.Logger().Error("Tabi block height is bounded by int64 range")
			return common.Hash{}
		}
		h := int64(height)
...
func (k *Keeper) PrepareReplayedAddr(ctx sdk.Context, addr common.Address) {
	...
	if a.Balance != nil && a.Balance.Cmp(utils.Big0) != 0 {
		atabi, wei := state.SplitAtabiWeiAmount(a.Balance)
		err = k.BankKeeper().AddCoins(ctx, k.GetTabiAddressOrDefault(ctx, addr), sdk.NewCoins(sdk.NewCoin("atabi", atabi)), true)
		if err != nil {
			panic(err)
		}
		err = k.BankKeeper().AddWei(ctx, k.GetTabiAddressOrDefault(ctx, addr), wei)
		if err != nil {
			panic(err)
		}
	}
