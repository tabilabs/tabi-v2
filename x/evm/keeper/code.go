...
func (k *Keeper) SetCode(ctx sdk.Context, addr common.Address, code []byte) {
	...
	// set association with direct cast Tabi address for the contract address
	if _, ok := k.GetTabiAddress(ctx, addr); !ok {
		k.SetAddressMapping(ctx, k.GetTabiAddressOrDefault(ctx, addr), addr)
	}
}
...
func (k *Keeper) GetCodeHash(ctx sdk.Context, addr common.Address) common.Hash {
	...
	if bz == nil {
		...
		if k.GetBalance(ctx, k.GetTabiAddressOrDefault(ctx, addr)).Cmp(utils.Big0) == 0 {
			return common.Hash{}
		}
