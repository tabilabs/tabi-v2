...
import (
	...
	artifactsutils "github.com/tabilabs/tabi-v2/x/evm/artifacts/utils"
...
func (k *Keeper) InitGenesis(ctx sdk.Context, genState types.GenesisState) {
	...
	tabiAddrFc := k.accountKeeper.GetModuleAddress(authtypes.FeeCollectorName) // feeCollector == coinbase
	k.SetAddressMapping(ctx, tabiAddrFc, GetCoinbaseAddress())

	for _, addr := range genState.AddressAssociations {
		k.SetAddressMapping(ctx, sdk.MustAccAddressFromBech32(addr.TabiAddress), common.HexToAddress(addr.EthAddress))
	}
