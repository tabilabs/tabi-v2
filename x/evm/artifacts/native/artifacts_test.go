...
func TestSimple(t *testing.T) {
	...
	tabiAddr, evmAddr := testkeeper.PrivateKeyToAddresses(privKey)
	...
	require.Nil(t, k.BankKeeper().SendCoinsFromModuleToAccount(ctx, types.ModuleName, tabiAddr, amt))
	require.Nil(t, k.BankKeeper().MintCoins(ctx, types.ModuleName, sdk.NewCoins(sdk.NewCoin("test", sdk.NewInt(200000000)))))
	require.Nil(t, k.BankKeeper().SendCoinsFromModuleToAccount(ctx, types.ModuleName, tabiAddr, sdk.NewCoins(sdk.NewCoin("test", sdk.NewInt(200000000)))))
	...
	_, found := k.GetTabiAddress(ctx, common.HexToAddress(receipt.ContractAddress))
