...
//go:embed NativeTabiTokensERC20.abi
//go:embed NativeTabiTokensERC20.bin
var f embed.FS
...
func GetABI() []byte {
	bz, err := f.ReadFile("NativeTabiTokensERC20.abi")
	if err != nil {
		panic("failed to read NativeTabiTokensERC20 contract ABI")
	}
	return bz
}
...
func GetBin() []byte {
	...
	code, err := f.ReadFile("NativeTabiTokensERC20.bin")
	if err != nil {
		panic("failed to read NativeTabiTokensERC20 contract binary")
	}
	bz, err := hex.DecodeString(string(code))
	if err != nil {
		panic("failed to decode NativeTabiTokensERC20 contract binary")
	}
	setCachedBin(bz)
