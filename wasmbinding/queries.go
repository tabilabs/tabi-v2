...
import (
	...
	evmbindings "github.com/tabilabs/tabi-v2/x/evm/client/wasm/bindings"
)
...
func NewQueryPlugin(evmh *evmwasm.EVMQueryHandler, sk stakingkeeper.Keeper) *QueryPlugin {
	return &QueryPlugin{
		evmHandler:    *evmh,
		stakingKeeper: sk,
	}
...
func (qp QueryPlugin) HandleEVMQuery(ctx sdk.Context, queryData json.RawMessage) (res []byte, err error) {
	...
	var parsedQuery evmbindings.TabiEVMQuery
	...
	case evmbindings.GetEvmAddressType:
		...
		return qp.evmHandler.HandleGetEvmAddress(ctx, c.TabiAddress)
	case evmbindings.GetTabiAddressType:
		c := parsedQuery.GetTabiAddress
		return qp.evmHandler.HandleGetTabiAddress(ctx, c.EvmAddress)
